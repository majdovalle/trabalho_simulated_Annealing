...
Num_Max_Viz=250;    % Numero maximo de iteraçoes em uma mesma temperatura
%(Numero de vizinhos a serem gerados nesta temperatura)

T_Ini=2300;       % Temp Inicial.

alpha=0.98;     % Taxa redução temperatura
num_it_sem_melhora=0;%numero de iteraçoes sem melhora
max_it_sem_melhora = 600; % maximo de iteraçoes sem melhora
%num_forces = 0; %numero de tentativas forçadas de melhora
%max_forces = 15; % 15 maximo de tentativas forçadas de melhora
...
%             num_it_sem_melhora=0;%numero de iteraçoes sem melhora
% %             break; %a busca por um vizinho melhor nesta temperatura é finalizada
%         else
...
%     %Armazena os melhores valores encontrados
    ...
    disp(['Iteração ' num2str(it) ': Melhor custo = ' num2str(MelhoresCustos(it)) ': Temperatura: ' num2str(T) ' Aceitos: ' num2str(aceita/Num_Max_Viz*100) '%']);
